...
public interface OrdersMapper {
   ...
   //找到所有已购商品
    @Select({
            ...
            "phonenum, fee, sendtime, receivetime",
            "from orders",
            "where order_state_id =1 limit #{pageNum},20"
    })
    @ResultMap("com.shop.dao.OrdersMapper.BaseResultMap")
    List<Orders> findBought(@Param("pageNum") int pageNum);

    @Select("select count(order_id) from orders where order_state_id =1")
    long count();
    ...
    @Select({
            ...
            "phonenum, fee, sendtime, receivetime",
            "from orders",
            "where user_id = #{userId,jdbcType=INTEGER} limit #{pageNum},20"
    })
    @ResultMap("com.shop.dao.OrdersMapper.BaseResultMap")
    List<Orders> findByUserId(@Param("userId") int userId, @Param("pageNum") int pageNum);

    int updateByPrimaryKeySelective(Orders record);

    //找到所有订单号
    @Select("select order_name from orders where user_id=#{userId} GROUP BY order_name")
